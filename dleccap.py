...
def get_cookie_and_site_id_from_ctools(url):
    ...
    try:
        ...
        for input_element in inputs:
            data_to_post[input_element.get(
        ...
        # get site ID
        soup = BeautifulSoup(p.text, "html.parser")
        recording_url = soup.find("div", {"class": "recording"}).find(
            "div", {"class": "play-link"}).find("a").get("href")
...
def authenticate(service=None):
    ...
    if not service:
        services = ["cosign-ctools",
                    "cosign-shibboleth.umich.edu", "cosign-leccap.engin"]
...
def main():
    ...
    print("%i recording%s available:" %
          (len(recordings), ("s" if (len(recordings) != 1) else "")))

    # print lecture info
    for index, recording in enumerate(recordings):
        print("%i\t(%s)\t%s" %
              (index + 1, recording["date"], recording["title"]))
    print

    # which recordings to download?
    print("Which recording would you like to download?")
    input_str = input(
        "Enter a number between %i and %i, or press Enter to download all: " % (1, len(recordings)))
    try:
        input_as_int = int(input_str)
    ...
    # download files
    for index, recording in enumerate(recordings):
        print("(%i/%i) Downloading %s ..." %
              (index + 1, len(recordings), recording["title"]))
